...
# 들어가며
계속 미루다 이제야 정리되었다. 이번 글은 JPA를 사용하다보면 절대 피하지 못하는 N + 1 문제에 대한 정리한 글이다.

가능한 처음 접하시는 분들도 쉽게 이해하실 수 있도록 작성하였다. 

개인적으로 직접 코드를 따라 작성하면서 읽으시길 추천드립니다. (N + 1은 더더욱 직접 코드를 작성하지 않으면 이해하기 어렵다고 생각합니다)

모든 소스 코드는 [여기](https://github.com/binghe819/learning-sandbox/tree/master/jpa-n-plus-1-basic)를 참고바랍니다.
...
실제 현업에선 일대일 관계가 아닌이상 위와 같은 상황은 거의 없다.
