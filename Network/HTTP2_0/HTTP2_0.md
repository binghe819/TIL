...
이로인해 TCP Connection이 idle 상태가 잦은 상황이 발생함으로써 Connection을 제대로 활용하지못하였다.

<p align="center"><img src="./image/http_1_1_pipelining_blocking.png"> </p>

Pipielining이라는 성능 향상 스펙이 존재했음에도, HTTP/1.1의 FIFO 형식의 처리한계로인해 HOLB (Head Of Line Blocking)이 발생하는등 결국 빛을 보지못하고 사용되지않게되었다.
...
물론 이 문제는 물리적인 TCP 연결을 여러 개 두는 방식으로 병렬적으로 처리할 수 있긴하지만 임시방편에 불가했다. 그 이유는 아래 TCP 내용을 보면 이해가 된다.
...
이것이 TCP가 성공할 수 있는 이유였으며, TCP가 속도는 조금 느렸어도 신뢰성있는 데이터를 통신하는데 큰 영향을 끼친 요소이기도하다.

이렇게 동작하는 가장 핵심은 TCP의 혼잡 제어 (Congestion Control)라고 볼 수 있다. 
...
HTTP/1.1의 응답에서 바디 크기대비 헤더 크기의 비율이 매우 낮지만, 요청에서는 헤더가 대부분의 용량을 차지할 수 있다.
...
결국 이 또한 전송해야하는 데이터의 증가로인해 성능적으로 좋지 않다.
...
💁‍♂️ **HTTP/1.1의 한계가 점점 드러났다**

1999년, HTTP/1.1을 기술한 RFC 2616에서 현대의 웹의 토대가 된 표준이 정의되었다.

그이후 지금까지도 사실상 HTTP/1.1은 대중적으로 사용되고있지만, 구현의 단순성과 접근성에 주안점을 두고 최적화한 프로토콜이다보니 성능측면에선 앞서 정리했뜻이 취약한 점이 많다.
...
**이 새로운 3가지 개념이 사실상 HTTP/2.0에도 그대로 이어지며, 사실상 성능 개선의 핵심 개념이다.**

> 이 세가지 개념에 대해선 아래 HTTP/2.0에 대해서 얘기할 때 다룰 예정이다.
...
HTTP의 애플리케이션 계층에서의 동작은 동일하며, 제공되는 기능이나 HTTP 메서드, 상태 코드, URI 및 헤더 필드와 같은 핵심 개념을 변경되지 않았다.

즉, 기존의 HTTP의 스펙은 HTTP 계층에서 동일하게 유지되며, 새로 생기는 SPDY 계층에서 HTTP/1.1의 성능 한계를 해결하고자한다는 것이다.

그 핵심이 바로 binary framing layer이다.
...
즉, 요청/응답 Frame이 각각 어떤 Stream에 속하는지 알 수 있기때문에 순서와 상관없이 요청/응답을 보냄으로써 병렬적으로 처리가 가능한 것이다.

> HTTP/1.1에선 응답이 요청을 알 수 있는 방법이 없어 순서대로 처리할 수 밖에 없다.

OS로 비유하자면, HTTP/1.1은 하나의 Connection내에서 동기 형태로 동작했따면, HTTP/2.0은 비동기처럼 동작한다.
...
즉, 서버는 클라이언트가 직접적으로 요청하지않은 리소스를 미리 꺼내서 가까운 미래에 특정 개체가 필요할 때 바로 사용될 수 있도록 성능 향상을 이끌어내는 것이다.
...
HTTP/2.0에선 HTTP 메세지를 여러 Binary Frame으로 분할하고, 여러 Frame을 멀티플렉싱 할 수 있게되면어 요청과 응답이 동시에 병렬적으로 이루어져 비약적인 속도 향상이 되었다.
...
gRPC등의 RPC 통신이 늘어남에따라 점차 HTTP/2.0의 사용량도 많아질 것으로 보이는데, HTTP/2.0도 여전히 문제가 존재한다.
