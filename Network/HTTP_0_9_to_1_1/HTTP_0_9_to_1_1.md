...
1989년 CERN에서 일하던 Tim Berners-Lee와 그의 팀은 대학과 연구소의 학술적 정보를 저장하고 교환하기위한 하이퍼텍스트 시스템을 만들기 위한 제안을 작생했다.

쉽게 말해 현재의 HTML파일을 주고받을 수 있는 시스템을 고안한 것인인데, 이는 초기에 Mesh라고 불렸으며 1990년에 구현 과정에서 월드 와이드 웹(World Wide Web, 이하 웹)으로 이름이 변경되었다고한다.
...
**이때 HTTP/1.0의 가장 큰 문제점이 나타나는데, 바로 HTTP의 비연결성 (connectionless) 특징으로인해 매 요청/응답마다 TCP 커넥션을 맺어야하는 Short-lived connections (단기 커넥션) 문제가 발생한다.**

즉, 커넥션 하나당 하나의 요청/응답 처리만 수행하며, 매 요청마다 새로운 TCP 커넥션을 맺는 구조였다.
...
HTTP/1.0의 가장 큰 문제는 Short-lived connections이다. 

이는 HTTP가 TCP 연결 기반위에서 신뢰성을 확보함과 동시에 비연결성을 만족시키기위해 매 요청/응답마다 커넥션을 맺고 끊기때문에 발생하는 문제이다.
...
서버의 응답이 순서 없이 반환될 경우 클라이언트는 어떤 요청에 대한 응답인지 알 수 없다는 의미이다.

이는 **아래와 같이 어떤 한 요청을 처리하는 데 시간이 오래 걸리는 경우, Head Of Line (HOL) Blocking 현상으로 인해 다른 요청들에 대한 응답도 함께 지연되는 문제가있다.**

그리고 아래 내용은 [HTTP/1.1 Pipelining 스펙](https://datatracker.ietf.org/doc/html/rfc2068#section-8.1.2.2)에 정의된 내용인데, 
...
**클라이언트와 서버가 각 상황별로 구현해야하는 부분이 까다롭다. 이로인해 인터넷상의 서버와 프록시중 파이프라이닝을 제대로 구현한 곳은 거의 없으며, 있더라도 제대로 동작하지 않는 경우가많아 현재는 잘 사용되지 않는다.**
...
결국 병렬 처리가 필요한데 HTTP/1.1에선 한계가 있으므로, 결국 브라우저는 물리적인 TCP 연결을 여러 개 두는 방식으로 병렬 연결을 구현하게되었다.
