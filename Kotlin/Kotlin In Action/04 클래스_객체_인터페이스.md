...
- [클래스 정의](#클래스-정의)
  - [코틀린 인터페이스](#코틀린-인터페이스)
  - [클래스 상속 제어 변경자 - open, final, abstract](#클래스-상속-제어-변경자---open-final-abstract)
  - [가시성 - public, internal, protected, private](#가시성---public-internal-protected-private)
  - [내부 클래스와 중첩 클래스](#내부-클래스와-중첩-클래스)
  - [sealed class](#sealed-class)
- [클래스 선언](#클래스-선언)
  - [주 생성자와 초기화 블록](#주-생성자와-초기화-블록)
  - [부 생성자](#부-생성자)
  - [getter, setter와 Backing Field](#getter-setter와-backing-field)
  - [접근자 가시성 변경](#접근자-가시성-변경)
- [데이터 클래스와 클래스 위임](#데이터-클래스와-클래스-위임)
  - [데이터 클래스](#데이터-클래스)
  - [클래스 위임 - by](#클래스-위임---by)
- [object - 클래스 선언과 인스턴스 생성](#object---클래스-선언과-인스턴스-생성)
  - [싱글턴 - singleton](#싱글턴---singleton)
  - [동반 객체 - companion object](#동반-객체---companion-object)
  - [무명 내부 클래스 - anonymous inner class](#무명-내부-클래스---anonymous-inner-class)
...
## 클래스 상속 제어 변경자 - open, final, abstract
...
코틀린도 이러한 철학은 따라서 코틀린의 클래스와 메서드는 모두 기본이 `final`이다.
...
이는 프로퍼티와 클래스의 상속 접근자라 `final`이여야한다는 의미이기도 하다. `final`이 아니면 하위 클래스가 존재하기때문에 스마트 캐스트를 못하기 때문.
...
* 주 생성자내 선언된 멤버변수의 디폴트 가시성은 `private`이다. val이면 final, var이면 final이 붙지안흔다.
* 생성자와 getter를 기본적으로 구현해준다.
...
그러므로 이랄 땐 초기화 블록이 필요하며, 초기화 블록은 `init` 키워드로 클래스의 객체가 만들어질 때 실행될 초기화 코드가 들어간다.
...
**클래스와 마찬가지로 객체 선언 안에도 프로퍼티, 메서드, 초기화 블록 등이 들어갈 수 잇다. 하지만 생성자 (주 생성자, 부 생성자 모두) 객체 선언에 쓸 수 없다.**
...
**한가지 눈 여겨봐야할 부분은 object 키워드를 사용하면 객체 내 `static{}` 에서 new를 이용한 객체생성과 변수 할당이 이루어진다.**
