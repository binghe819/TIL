...
#### :+1: 6. 생성자는 객체를 생성하고 값을 set하는 반면, static 팩토리 메서드는 객체 생성전에 값의 유효성을 검사할 수 있다.

생성자는 객체를 생성하고 값을 set한다. 이때 값을 유효성 검사한다한들, 이미 객체를 생성되었기때문에 메모리와 GC의 낭비가 발생한다.

**반면, static 팩토리 메서드는 객체를 생성하기전에 값의 유효성 검사를 먼저 할 수 있다.**
