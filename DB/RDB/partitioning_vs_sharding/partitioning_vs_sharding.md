...
💁‍♂️ **샤딩은 Hotizontal (수평) 파티셔닝의 일종이다.**
...
> 샤딩과 수평 파티셔닝의 차이점
> * 수평 파티셔닝은 모든 데이터를 동일한 서버에 저장한다.
> * 샤딩은 데이터를 서로 다른 컴퓨터에 분산한다.
...
위키 백과에도 잘 정의되었듯이, 샤딩으 동일한 스키마를 가지고 여러 대의 데이터베이스 서버들에 데이터를 작은 단위로 나누어 분산 저장하는 방법이다.
